...
def get_transforms(version: str) -> List[Compose]:
    if version == 'color':
        ...
        transforms = [
            ...
            dict(type='BgShearY'),
            dict(type='BgRotate'),
            dict(type='BgTranslateX'),
            dict(type='BgTranslateY'),
            dict(type='BBoxShearX'),
            dict(type='BBoxShearY'),
            dict(type='BBoxRotate'),
            dict(type='BBoxTranslateX'),
            dict(type='BBoxTranslateY'),
        ]
    elif version == 'oamix':
        transforms = [
            dict(type='AutoContrast'),
        ...
        raise TypeError(
            ...
            f'Please add the version to the get_transforms function.')
    transforms = [Compose(transforms) for transforms in transforms]
    return transforms


def bbox_overlaps_np(bboxes1: np.ndarray,
                     bboxes2: np.ndarray,
...
class OAMix(BaseTransform):
    ...
    Args:
        version (str): The version of the augmentation method.
            Defaults to 'oamix'.
        aug_prob_coeff (float):
            The coefficient of the augmentation probability.
            Defaults to 1.0.
        mixture_width (int):
            The number of augmentation operations in the mixture.
            Defaults to 3.
        mixture_depth (int):
            The depth of augmentation operations in the mixture.
    ...
    def __init__(self,
        ...
        assert version in ['color', 'geo', 'oamix'], \
            "The version should be either 'color', 'geo', or 'oamix'."\
            'Please add the version to the get_transforms function.'
        assert aug_prob_coeff > 0, \
            'The augmentation probability coefficient ' \
            'should be greater than 0.'
        assert isinstance(
            mixture_width, int
        ) and mixture_width > 0, 'The mixture width should be greater than 0.'
        assert isinstance(
            mixture_depth, int
        ) and mixture_depth >= -1, \
            'The mixture depth should be greater than or equal to -1.'
        assert isinstance(box_scale, tuple) and len(
            box_scale) == 2, 'The box scale should be a tuple of 2 elements.'
        assert isinstance(box_ratio, tuple) and len(
            box_ratio) == 2, 'The box ratio should be a tuple of 2 elements.'
        assert 0 <= sigma_ratio <= 1, \
            'The sigma ratio should be in the range [0, 1].'
        assert score_thresh >= 0, \
            'The score threshold should be greater than or equal to 0.'
    ...
    def oamix(self, img_orig: np.ndarray, gt_bboxes: np.ndarray,
        ...
        for i in range(self.mixture_width):
            ...
            depth = self.mixture_depth \
                if self.mixture_depth > 0 else np.random.randint(1, 4)
            img_aug = img_orig.copy()
            for _ in range(depth):
                img_aug = self.multilivel_transform(img_aug, gt_bboxes,
                                                    gt_masks)
            img_mix += ws[i] * img_aug
        """ Object-aware mixing """
        img_oamix = self.object_aware_mixing(img_orig, img_mix, gt_bboxes,
                                             gt_masks)

        return np.asarray(img_oamix, dtype=img_orig.dtype)

    def multilivel_transform(self, img: np.ndarray, gt_bboxes: np.ndarray,
                             gt_masks: List[np.ndarray]) -> np.ndarray:
        ...
        img_aug = np.asarray(
            img_tmp + (1.0 - union_mask) * self.aug(img, gt_bboxes, gt_masks),
            dtype=img.dtype)
        return img_aug

    def get_random_bboxes(self,
                          img_shape: Tuple,
                          num_bboxes: Tuple[int, int],
                          max_iters: int = 50,
                          eps: float = 1e-6) -> np.ndarray:
        assert max_iters > 0, \
            'The maximum number of iterations should be greater than 0.'
    ...
    def get_masks(self,
                  bboxes: np.ndarray,
                  img_shape: tuple,
                  use_blur: bool = False) -> List[np.ndarray]:
            ...
            if use_blur:
                ...
                if not (sigma_x <= 0 or sigma_y <= 0):
                    mask = cv2.GaussianBlur(
                        ...
                        sigmaY=sigma_y.item())
                mask = cv2.resize(
                    mask, (img_shape[1], img_shape[0]),
                    interpolation=cv2.INTER_LINEAR)
            mask_list.append(mask)
        return mask_list

    def aug(self, img: np.ndarray, gt_bboxes: np.ndarray,
    ...
    def object_aware_mixing(self, img_orig: np.ndarray, img_mix: np.ndarray,
        ...
        target_m = np.random.uniform(0.0, 0.5,
                                     len(target_bboxes)).astype(np.float32)
        ...
        rand_m = np.random.uniform(0.0, 1.0,
                                   len(rand_bboxes)).astype(np.float32)
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'(version={self.version}, ' \
                    f'aug_prob_coeff={self.aug_prob_coeff}, ' \
                    f'mixture_width={self.mixture_width}, ' \
                    f'mixture_depth={self.mixture_depth}, ' \
                    f'box_scale={self.box_scale}, ' \
                    f'box_ratio={self.box_ratio}, ' \
                    f'sigma_ratio={self.sigma_ratio}, ' \
                    f'score_thresh={self.score_thresh})'
        return repr_str

    @staticmethod
    def _load_random_data() -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _test_transformations(self) -> None:
        img, gt_bboxes = self._load_random_data()
        gt_masks = self.get_masks(gt_bboxes, img.shape, use_blur=True)

        transform_type_list = [
            'AutoContrast', 'Brightness', 'Color', 'Contrast', 'Equalize',
            'Invert4Mix', 'Posterize', 'Sharpness', 'Solarize', 'SolarizeAdd',
            'BgShearX', 'BgShearY', 'BgRotate', 'BgTranslateX', 'BgTranslateY',
            'BBoxShearX', 'BBoxShearY', 'BBoxRotate', 'BBoxTranslateX',
            'BBoxTranslateY'
        ]
    ...
    def _test_multilevel_transformations(self) -> None:
        img_orig, gt_bboxes = self._load_random_data()
        gt_masks = self.get_masks(gt_bboxes, img_orig.shape, use_blur=True)

        for idx in range(10):
            ws = np.float32(
                np.random.dirichlet([self.aug_prob_coeff] *
                                    self.mixture_width))
            img_mix = np.zeros_like(img_orig, dtype=np.float32)
            for i in range(self.mixture_width):
                """Multi-level transformation."""
                ...
                img_aug = img_orig.copy()
                for _ in range(depth):
                    img_aug = self.multilivel_transform(
                        img_aug, gt_bboxes, gt_masks)
                img_mix += ws[i] * img_aug
    ...
    def _test_objectaware_mixing(self) -> None:
        ...
        for i in range(self.mixture_width):
            """Multi-level transformation."""
            depth = self.mixture_depth if \
                self.mixture_depth > 0 else np.random.randint(1, 4)
            img_aug = img_orig.copy()
            for _ in range(depth):
                img_aug = self.multilivel_transform(img_aug, gt_bboxes,
                                                    gt_masks)
            img_mix += ws[i] * img_aug
        """ Object-aware mixing """
        self.object_aware_mixing(img_orig, img_mix, gt_bboxes, gt_masks)
